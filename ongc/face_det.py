...
dialog_title='QUIT'
dialog_text='Are u sure?'
window.configure(background='blue')
window.grid_rowconfigure(0,weight=1)
window.grid_columnconfigure(0,weight=1)

message=tk.Label(window,text="Face Recognisation Based Attendance Management System",bg="Green",fg="White",width=50,height=3,font=('times',30,'italic bold underline'))
...
lbl3=tk.Label(window,text="Notifications",width=20,fg="red",bg="yellow",height=2,font=('times',15,'bold'))
...
def TakeImages():
    ...
    if(is_number(Id) and name.isalpha()):
        ...
        harcascadePath="C:\Python\haarcascade_frontalface_default.xml"
        detector=cv2.CascadeClassifier(harcascadePath)
        sampleNum=0
        while(True):
            ret,img=cam.read()
            gray=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            faces=detector.detectMultiScale(gray,1.3,5)##faces=detector.detectMultiScale(gray,1.3,5)
        ...
        row=[Id,name]
        with open('C:\Python\StudentDetails\StudentDetails.csv','a+') as csvFile:
...
def TrainImages():
    ...
    harcascadePath="C:\Python\haarcascade_frontalface_default.xml"
    detector=cv2.CascadeClassifier(harcascadePath)
    faces,Id=getImagesAndLabels("C:\Python\TrainingImages")
    recognizer.train(faces,np.array(Id))
    recognizer.save("C:\Python\TrainingImageLabel\Trainner.yml")
...
def TrackImages():
    ...
    recognizer.read("C:\Python\TrainingImageLabel\Trainner.yml")
    harcascadePath="C:\Python\haarcascade_frontalface_default.xml"
    ...
    while True:
        ...
        faces=faceCascade.detectMultiScale(gray,1.2,5)##faces=faceCascade.detectMultiScale(gray,1.2,5)
        for (x,y,w,h)in faces:
    ...
    date=datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
    timeStamp=datetime.datetime.fromtimestamp(ts).strftime('%H:%M,%S')
    Hour,Minute=timeStamp.split(":")#Hour,Minute,Second=timeStamp.split(":")
    fileName="Attendance\Attendance_"+date+"_"+Hour+"-"+Minute+"-"+".csv"#fileName="Attendance\Attendance_"+date+"_"+Hour+"-"+Minute+"-"+Second+".csv"
...
clearButton2=tk.Button(window,text="Clear",command=clear,fg="red",bg="yellow",width=20,height=2,activebackground ="Red",font=('times',15,'bold'))
